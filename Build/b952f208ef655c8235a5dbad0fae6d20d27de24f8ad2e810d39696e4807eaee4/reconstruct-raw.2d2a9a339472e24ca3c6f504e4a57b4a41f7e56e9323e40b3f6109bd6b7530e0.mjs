...
export function createPublicAssetReader({baseUrl=GITHUB_BASE,fetchImpl=globalThis.fetch,allowLocalPreview=false}={}){
  ...
  return async function readSource(file,signal){
    ...
    // Fetch exposes decoded body bytes, but Content-Length describes the
    // transferred (possibly gzip/br-compressed) representation on a CDN.
    const length=response.headers?.get('Content-Length');
    const encoding=response.headers?.get('Content-Encoding')?.trim().toLowerCase();
    if(length!==null&&length!==undefined){
      check(/^\d+$/.test(length),'Invalid response declared length');
